...
from tokenizer import BPE, train_bpe
...
# Text generation functions modified to work with BPE tokenizer
...
def top_k_decode(model, context, max_new_tokens, tokenizer, device, k=50, temperature=1.0):
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            top_k_logits, top_k_indices = torch.topk(next_token_logits, min(k, next_token_logits.size(-1)), dim=-1)
...
def main():
    ...
    vocab_size = 32000  # Target vocabulary size for BPE
    ...
    if os.path.exists(tokenizer_path):
        ...
        tokenizer = BPE.load(tokenizer_path)
    else:
        print("Training BPE tokenizer...")
        tokenizer = train_bpe(dataset["train"], vocab_size=vocab_size, output_path=tokenizer_path)
    ...
    actual_vocab_size = tokenizer.vocab_size()  # Use the actual vocabulary size from our tokenizer
    ...
    model = Transformer(d_model, num_heads, max_length, actual_vocab_size, layers, bias=False, dropout=dropout)
